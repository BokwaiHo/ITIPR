...
class Read_data(DatasetLoader):
    ...
    def load(self):
        ...
        elif "movielens" in self.fpath:
            df = pd.read_csv(self.fpath,
                             sep=',',
                             engine='python',
                             names=['userId', 'movieId', 'rating', 'timestamp'])
            df = df.sort_values(by='userId').reset_index().drop(['index'], axis=1)
            df = df[['userId', 'movieId', 'rating']].rename(columns={"userId": "user", "movieId": "item"})
...
class Data(object):
    def __init__(self, data_dir, data_name, infrequent_threshold=5, val_ratio=None, test_ratio=0.2, user_filter=5, item_filter=5, clean='N',
                 seed=0):
        ...
        if os.path.exists(file_path):
            ...
            if data_name == 'mv100':
                ...
                df = Read_data(data_dir).load() #Amazon-Books, Amazon-CDs, Movielens-latest, Movielens-20M
        ...
        print("make sure each user has {} real preference ratings...".format(str(infrequent_threshold)))
        df_real = self.remove_infrequent_users(df_real, infrequent_threshold) #books:20 others:5 
